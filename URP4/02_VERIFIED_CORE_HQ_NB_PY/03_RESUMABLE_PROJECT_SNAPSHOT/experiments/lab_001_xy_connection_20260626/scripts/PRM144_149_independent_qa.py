...
R=Path(__file__).resolve().parents[3];T=R/'experiments/lab_001_xy_connection_20260626/reports/tables';O=R/'experiments/lab_001_xy_connection_20260626/factories/PRM-149/reports';O.mkdir(parents=True,exist_ok=True)
...
def main():
 r=[c(144,145,'v2_5',143,'v2_4'),c(146,147,'v2_6',145,'v2_5'),c(148,149,'v2_7',147,'v2_6')];ok=all(x['finite']==x['candidates']*58 and x['replay_prior'] and x['replay_cohort'] and x['no_y'] for x in r);q=pd.DataFrame(r);q['status']='PASS' if ok else 'FAIL';q.to_csv(O/'PRM144_149_independent_QA.csv',index=False,encoding='utf-8-sig');s={'status':'PASS' if ok else 'FAIL','cycles':3,'checks':'3/3' if ok else 'failed','final_bank':r[-1]['bank'],'final_values':r[-1]['values'],'performance_y_read':0};(O/'PRM144_149_independent_QA_summary.json').write_text(json.dumps(s,indent=2),encoding='utf8');print(json.dumps(s));
