"""Deterministic source-equivalent binary STL export and mesh QA."""
...
from .models import ExportAttempt, TPMSGenerationResult


def write_binary_stl(
    path: str | Path, vertices: np.ndarray, faces: np.ndarray, solid_name: str = "mesh"
) -> Path:
    output = Path(path)
...
def export_stl(result: TPMSGenerationResult, path: str | Path) -> ExportAttempt:
    ...
    return ExportAttempt(
        ...
        backend_id="SOURCE-BINARY-STL-MARCHING-CUBES-v0.1",
        notes="Binary STL writer and marching-cubes array order match immutable notebook selected functions.",
    )
...
def mesh_qa(path: str | Path) -> dict[str, object]:
    ...
    if not finite or boundary > 0:
        technical = "fail_open_or_invalid"
        topology = "open_or_invalid"
    elif nonmanifold > 0:
        technical = "pass_with_topology_warning"
        topology = "source_equivalent_nonmanifold_marching_cubes"
    else:
        technical = "pass"
        topology = "closed_2_manifold"
    return {
