...
The route policy is machine-readable in [source policy](../LINK_TARGETS_R2/ROUTE-VALID-004-20260729-001_source_type_route_policy.csv). The key rule is: route eligibility is a property of the source identity and evidence level, not a convenience switch.
...
The [fallback table](../LINK_TARGETS_R2/ROUTE-VALID-004-20260729-001_fail_closed_fallback.csv) enumerates required hard stops. The controller must stop rather than choose a permissive route when source identity, hash, transform, orientation or required warning status is missing. A–B must never be converted into an independent imported-STL claim.
